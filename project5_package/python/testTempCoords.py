...
pts1 = pts['pts1']
pts2 = np.concatenate([epipolarCorrespondence(img1, img2, F, np.array([pt1])) for pt1 in pts1], axis=0)
pts3d = triangulate(np.dot(K1, P1), pts1, np.dot(K2, P2), pts2)
fig = plt.figure(figsize=(12, 6))
ax = fig.add_subplot(1, 2, 1, projection='3d')
ax.scatter(xs=pts3d[:, 0], ys=pts3d[:, 1], zs=pts3d[:, 2], s=3)
