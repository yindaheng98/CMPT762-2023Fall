...
class StemConv(nn.Module):
    def __init__(self, in_channels, out_channels, norm_cfg=dict(type='SyncBN', requires_grad=True)):
        ...
        self.proj = nn.Sequential(
            ...
            nn.GroupNorm(norm_cfg["num_groups"], out_channels // 2),
            nn.GELU(),
...
class Block(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.norm1 = nn.GroupNorm(norm_cfg["num_groups"], dim)
        self.attn = SpatialAttention(dim)
        self.drop_path = DropPath(
            drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = nn.GroupNorm(norm_cfg["num_groups"], dim)
...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=7, stride=4, in_chans=3, embed_dim=768, norm_cfg=dict(type='SyncBN', requires_grad=True)):
        ...
        self.norm = nn.GroupNorm(norm_cfg["num_groups"], embed_dim)
